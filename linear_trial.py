...
class LinearGaussianPolicy(nn.Module):
    def __init__(self, obs_dim, act_dim, log_std_min=-20, log_std_max=2):
        super(LinearGaussianPolicy, self).__init__()
        self.log_std_min = log_std_min
        self.log_std_max = log_std_max
        
        self.mean_linear = nn.Linear(obs_dim, act_dim)
        self.log_std_linear = nn.Linear(obs_dim, act_dim)
        
        # Optional initialization
        nn.init.zeros_(self.mean_linear.weight)
        nn.init.zeros_(self.mean_linear.bias)
        nn.init.zeros_(self.log_std_linear.weight)
        nn.init.zeros_(self.log_std_linear.bias)
        
    def forward(self, obs):
        mean = self.mean_linear(obs)
    ...
    def log_prob(self, obs, action):
        ...
        # If a = tanh(x), then x = arctanh(a)
        # arctanh(a) = 0.5 * log((1+a)/(1-a))
        ...
        log_prob_gaussian = log_prob_gaussian.sum(axis=-1, keepdim=True)
        
        # Change of variables (Jacobian of the tanh)
        # log|det(d a/d x)| = sum of log(1 - a^2)
...
def main():
    ...
    # Initialize networks
    actor = LinearGaussianPolicy(obs_dim, act_dim).to(device)
    ...
    for step in range(max_steps):
        ...
        if done:
            state, _ = env.reset()
            episode_count += 1
            # print("Episode:", episode_count, "Return:", episode_reward, "Steps:", episode_steps)
            episode_steps = 0
            episode_reward = 0
