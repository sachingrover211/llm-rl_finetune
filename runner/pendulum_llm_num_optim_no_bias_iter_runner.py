...
from agent.pendulum_llm_num_optim_no_bias_iterative import PendulumLLMNumOptimAgent
...
def run_training_loop(
    ...
    llm_1st_round_template_name,
    llm_round_template_name,
    llm_model_name,
    ...
    assert task == "pendulum_llm_num_optim_no_bias_iterative"

    jinja2_env = Environment(loader=FileSystemLoader(template_dir))
    llm_si_template = jinja2_env.get_template(llm_si_template_name)
    llm_1st_round_template = jinja2_env.get_template(llm_1st_round_template_name)
    llm_round_template = jinja2_env.get_template(llm_round_template_name)

    world = PendulumContinuousWorld(
        gym_env_name,
        render_mode,
        max_traj_length,
    )
    agent = PendulumLLMNumOptimAgent(
        logdir,
        dim_actions,
        dim_states,
        max_traj_count,
