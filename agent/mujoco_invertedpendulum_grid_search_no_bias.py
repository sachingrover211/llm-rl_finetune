...
from world.mujoco_invertedpendulum import MujocoInvertedPendulumWorld
import numpy as np
import re
import itertools


class MujocoInvertedpendulumGridSearchAgent:
    def __init__(
        self,
        logdir,
        dim_action,
        ...
        grid = np.arange(-10, 10, 2)
        self.grids = list(itertools.product(grid, grid, grid, grid))
        self.counter = 0

    def rollout_episode(self, world: MujocoInvertedPendulumWorld, logging_file, record=True):
        state = world.reset()
    ...
    def random_warmup(self, world: MujocoInvertedPendulumWorld, logdir, num_episodes):
    ...
    def train_policy(self, world: MujocoInvertedPendulumWorld, logdir, search_std):

        def str_4d_examples(replay_buffer: EpisodeRewardBufferNoBias):
        ...
        reasoning = str_4d_examples(self.replay_buffer)
        new_parameter_list = np.array(self.grids[self.counter]).reshape((4, 1))
        self.counter += 1
    ...
    def evaluate_policy(self, world: MujocoInvertedPendulumWorld, logdir):
        results = []
