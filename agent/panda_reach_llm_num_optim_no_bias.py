...
from world.panda_reach import PandaReachWorld
import numpy as np
import re


class PandaReachLLMNumOptimAgent:
    def __init__(
        self,
        logdir,
        dim_action,
    ...
    def rollout_episode(self, world: PandaReachWorld, logging_file, record=True):
        state = world.reset()
    ...
    def random_warmup(self, world: PandaReachWorld, logdir, num_episodes):
    ...
    def train_policy(self, world: PandaReachWorld, logdir, search_std):
        ...
        def parse_parameters(input_text):
            ...
            return np.array(results).reshape((9, 3))

        def str_27d_examples(replay_buffer: EpisodeRewardBufferNoBias):
        ...
        new_parameter_list, reasoning = self.llm_brain.llm_update_parameters_num_optim(
            str_27d_examples(self.replay_buffer),
            parse_parameters,
    ...
    def evaluate_policy(self, world: PandaReachWorld, logdir):
        results = []
