...
from world.mountain_car_continuous import MountainCarContinuousWorld
import numpy as np


class MountaincarSASAgent:
    def __init__(
        self,
        logdir,
        dim_action,
    ...
    def rollout_episode(
        self, world: MountainCarContinuousWorld, logging_file, record=True
    ):
        state = world.reset()
        ...
        if record:
            self.replay_buffer.add(self.policy.weight, self.policy.bias, world.get_accu_reward())
        return world.get_accu_reward()

    def random_warmup(self, world: MountainCarContinuousWorld, logdir, num_episodes):
    ...
    def train_policy(self, world: MountainCarContinuousWorld, logdir):

        def parse_parameters(parameters_str):
            try:
                parameters = parameters_str.replace("[", "").replace("]", "").strip().split()
                parameters = np.array([float(x) for x in parameters]).reshape(3, 3)
                return parameters
            except Exception as e:
                print(f"Error parsing parameters: {e}")
                parameters = np.zeros((3, 3))
                return None
        ...
        new_parameter_list, (reasoning, reasoning_processed) = (
            self.llm_brain.llm_update_parameters_sas(
                self.replay_buffer, parse_parameters
            )
        )
    ...
    def evaluate_policy(self, world: MountainCarContinuousWorld, logdir):
        results = []
