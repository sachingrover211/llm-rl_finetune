...
class MountaincarContinuousActionGridSearchAgent:
    def __init__(
        self,
        logdir,
        dim_action,
        ...
        grid = np.arange(-10, 10, 1)
        self.grids = list(itertools.product(grid, grid))
        self.counter = 0
    ...
    def train_policy(self, world: MountaincarContinuousActionWorld, logdir, search_std):
        ...
        reasoning = str_2d_examples(self.replay_buffer)
        new_parameter_list = np.array(self.grids[self.counter]).reshape((2, 1))
        self.counter += 1
