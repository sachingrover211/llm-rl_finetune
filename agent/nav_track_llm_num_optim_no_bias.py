...
from world.nav_track import NavTrackWorld
import numpy as np
import re


class NavTrackLLMNumOptimAgent:
    def __init__(
        self,
        logdir,
        dim_action,
    ...
    def rollout_episode(self, world: NavTrackWorld, logging_file, record=True):
        state = world.reset()
    ...
    def random_warmup(self, world: NavTrackWorld, logdir, num_episodes):
    ...
    def train_policy(self, world: NavTrackWorld, logdir, search_std):
        ...
        def parse_parameters(input_text):
            ...
            return np.array(results).reshape((5, 3))

        def str_15d_examples(replay_buffer: EpisodeRewardBufferNoBias):
        ...
        new_parameter_list, reasoning = self.llm_brain.llm_update_parameters_num_optim(
            str_15d_examples(self.replay_buffer),
            parse_parameters,
    ...
    def evaluate_policy(self, world: NavTrackWorld, logdir):
        results = []
