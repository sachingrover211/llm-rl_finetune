...
class MountaincarContinuousActionLLMNumOptimImitationAgent:
    def __init__(
        self,
        logdir,
        dim_action,
        ...
        self.imitation_demonstrations_dir = imitation_demonstrations_dir
        self.imitation_demonstrations = self.load_imitation_demonstrations(
            imitation_demonstrations_dir
        )
        self.demonstrations_str = self.convert_demonstrations_to_str(
            self.imitation_demonstrations
        )

    def convert_demonstrations_to_str(self, demonstrations):
        x = []
        f_x = []
        for demonstration in demonstrations:
            for state_action in demonstration:
                ...
                f_x.append(action)
        demonstrations_str = ""
        for i in range(max(len(x), 100)):
            demonstrations_str += f"x: [{x[i][0]:.5g},{x[i][1]:.5g}]; f(x): {f_x[i][0]:.5g}\n"
        return demonstrations_str

    def load_imitation_demonstrations(self, imitation_demonstrations_dir):
        imitation_demonstrations = []
        for i in range(1):
            filename = f"{imitation_demonstrations_dir}/episode_{i+1}.json"
            with open(filename, "r") as f:
                data = json.load(f)
                imitation_demonstrations.append(data)
                f.close()
        return imitation_demonstrations

    def rollout_episode(
        self, world: MountaincarContinuousActionWorld, logging_file, record=True
    ):
        state = world.reset()
    ...
    def random_warmup(
        self, world: MountaincarContinuousActionWorld, logdir, num_episodes
    ):
    ...
    def train_policy(self, world: MountaincarContinuousActionWorld, logdir, search_std):
        ...
        new_parameter_list, reasoning = self.llm_brain.llm_update_parameters_num_optim_imitation(
            self.demonstrations_str,
