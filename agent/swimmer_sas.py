...
class SwimmerSASAgent:
    ...
    def rollout_episode(
        self, world: SwimmerContinuousWorld, logging_file, record=True
    ):
        state = world.reset()
    ...
    def train_policy(self, world: SwimmerContinuousWorld, logdir):

        def parse_parameters(parameters_str):
            try:
                parameters = parameters_str.replace("[", "").replace("]", "").strip().split()
                parameters = np.array([float(x) for x in parameters]).reshape(9, 2)
                return parameters
