...
class PendulumGridSearchAgent:
    def __init__(
        ...
        self.grids = list(itertools.product(grid, grid, grid))
        self.counter = 0

    def rollout_episode(self, world: PendulumContinuousWorld, logging_file, record=True):
        state = world.reset()
    ...
    def train_policy(self, world: PendulumContinuousWorld, logdir, search_std):
        ...
        reasoning = str_3d_examples(self.replay_buffer)
        new_parameter_list = np.array(self.grids[self.counter]).reshape((3, 1))
        self.counter += 1
