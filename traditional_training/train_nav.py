...
def train_linear_policy_cem(
    ...
    obs_dim = 5  # Should be 11 for Hopper-v5
    act_dim = 3  # Should be 3 for Hopper-v5

    # Verify dimensions
    assert obs_dim == 5, f"Expected obs_dim=11, got {obs_dim}"
    ...
    log_file = open("logs/nav_cem_log.txt", "w")
    for iteration in range(num_iterations):
        ...
        for _ in range(population_size):
            ...
            W_candidate = mu + sigma * np.random.randn(obs_dim, act_dim)
            # Evaluate candidate
...
def evaluate_linear_policy_with_bias(env, params, num_episodes=1, max_steps=1000):
    ...
    W = W_flat.reshape(3, 11)
    b = b_flat.reshape(3,)  # or just b_flat
...
def train_hopper_cma_es_with_bias():
    # Hyperparameters
    ENV_NAME = "Hopper-v5"
    SEED = 123
    MAX_GENERATIONS = 50
    ...
    W_flat = best_params[:33]
    b_flat = best_params[33:]
    print("W:\n", W_flat.reshape(11, 3))
    print("b:\n", b_flat)
