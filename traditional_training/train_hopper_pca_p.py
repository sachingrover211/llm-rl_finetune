...
def train_linear_policy_cma_es(env_name="Hopper-v5",
    ...
    log_file = open("logs/cma_es_log.txt", "w")
    # CMA-ES main loop
    while not es.stop():
        iteration += 1
...
def train_linear_policy_with_pca(env_name="Hopper-v5",
                               seed=123,
                               num_iterations=10,
                               population_size=16,
                               init_sigma=0.5,
    ...
    random_policies = np.random.randn(n_random_policies, obs_dim * act_dim)

    pca = PCA(n_components=pca_components)
    pca.fit(random_policies)

    param_dim = pca_components
    init_params = np.zeros(param_dim)  # start from zero matrix
    ...
    def evaluate_pca_policy(params):
        # print(params)
        params = pca.inverse_transform(params)
        # print("AFTERR")
        # print(params)
        return evaluate_policy(env, params,
                               num_episodes=rollout_episodes,
                               max_steps=max_steps)
    ...
    log_file = open("logs/cma_es_hopper_pca_p_log.txt", "w")
    # CMA-ES main loop
    while not es.stop():
        iteration += 1
        solutions = es.ask()
        ...
        for sol in solutions:
            # Evaluate on environment
            reward = evaluate_pca_policy(sol)
            rewards.append(-reward)  
            # CMA-ES is a minimizer by default, 
            # so we use the negative of the reward as the "cost"
            # Track best solution
    ...
    # Final best solution
    best_params = pca.inverse_transform(best_params)
    return best_params, best_score

def train_hopper_cma_es():
    ...
    # Exp2
    best_params, best_return = train_linear_policy_with_pca(
            env_name=ENV_NAME,
            seed=SEED,
