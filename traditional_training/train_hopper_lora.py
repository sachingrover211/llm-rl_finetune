...
def train_linear_policy_cma_es(env_name="Hopper-v5",
    ...
    rank = 4
    param_dim = (obs_dim + act_dim) * rank  # 11*3=33
    ...
    log_file = open("logs/cma_es_hopper_lora_log.txt", "w")
    # CMA-ES main loop
    while not es.stop():
        iteration += 1
        ...
        for sol in solutions:
            ...
            sol = sol.reshape(obs_dim + act_dim, rank)
            sol = np.dot(sol[:obs_dim].reshape(obs_dim, rank), sol[obs_dim:].reshape(rank, act_dim))
