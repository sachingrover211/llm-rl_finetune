...
def train_hopper_cma_es(task, exp_id, rank):
...
def train_hopper_cma_es_with_bias(task, exp_id, rank):
    # Hyperparameters
    ENV_NAME = task
    SEED = np.random.randint(0, 10000)
    MAX_GENERATIONS = 400
...
if __name__ == "__main__":
    ...
    def convert_bias(bias):
        if bias.lower() == "true":
            return True
        elif bias.lower() == "false":
            return False
        else:
            raise ValueError("bias must be either True or False")
    
    def convert_task(task):
        if task.lower() == "hopper":
            return "Hopper-v5"
        elif task.lower() == "halfcheetah":
            return "HalfCheetah-v5"
        elif task.lower() == "swimmer":
            return "Swimmer-v5"
        elif task.lower() == "walker":
            return "Walker2d-v5"
        elif task.lower() == "ant":
            return "Ant-v5"
    ...
    if len(sys.argv) != 5:
        print("Usage: python train_hopper_random_projection.py bias task exp_id rank")
        sys.exit(1)

    bias = convert_bias(sys.argv[1])
    task = convert_task(sys.argv[2])
    exp_id = int(sys.argv[3])
    rank = int(sys.argv[4])

    if not bias:
        train_hopper_cma_es(task, exp_id, rank)
    else:
        train_hopper_cma_es_with_bias(task, exp_id, rank)