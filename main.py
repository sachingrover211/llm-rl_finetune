import yaml
import argparse
from runner import walker_2d_runner, mountain_car_runner, pendulum_runner, blackjack_runner, cartpole_runner, frozen_lake_runner, hopper_runner, mountain_car_cont_runner, inv_pendulum_runner, pong_runner, swimmer_runner
...
def main():
    ...
    elif config["task"] == "inverted_pendulum":
        inv_pendulum_runner.run_training_loop(**config)
    elif config["task"] == "pong":
        pong_runner.run_training_loop(**config)
    elif config["task"] == "swimmer":
        swimmer_runner.run_training_loop(**config)
    elif config["task"] == "walker":
        walker_2d_runner.run_training_loop(**config)
    else:
