...
class MountainCarWorld(BaseWorld):
    ...
    def discretize_state(self, state):
        if not self.tile_state:
            ...
            position_idx = (
                np.digitize(state[0], self.position_bins)
            )
            velocity_idx = (
                np.digitize(state[1], self.velocity_bins)
            )
            return position_idx * self.num_velocity_bins + velocity_idx
